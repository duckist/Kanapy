...
class ReminderButton(
    ...
    @classmethod
    async def for_user(cls, bot: Bot, anime_id: int, user_id: int) -> Self:
    ...
    async def callback(self, interaction: discord.Interaction[Bot]):  # pyright: ignore[reportIncompatibleMethodOverride]
        toggled = await self.toggle_reminder(
            interaction.client, self.anime_id, self.user_id
        )

        if self.user_id == interaction.user.id:
            self.item.style = (
                discord.ButtonStyle.green if toggled else discord.ButtonStyle.gray
            )

            return await interaction.response.edit_message(view=self.view)

        await interaction.response.send_message(
            "Reminder set! I will remind you when a future episode airs."
            if toggled
...
class RelationSelect(ui.Select[ui.View]):
    ...
    async def callback(self, interaction: discord.Interaction["Bot"]):  # type: ignore
        ...
        await interaction.followup.send(
            ...
            ephemeral=True,
            view=View.from_result(interaction, result),  # pyright: ignore[reportArgumentType]
        )
...
class Frontend(BaseCog):
    ...
    async def anime_search(self, interaction: discord.Interaction[Bot], query: str):
        ...
        embed = AnimangaEmbed.from_result(result)
        view = View.from_result(interaction, result)
